...
// Last generated: 2026-03-04T21:40:49.605Z
...
<body class="bg-slate-900 text-slate-100 min-h-screen font-sans selection:bg-green-500 selection:text-white">
  ...
  <script>
    ...
    function renderStatCards() {
        ...
        const inTransitCount = allShipments.filter(s => s.status === 'PICKED_UP' && s.urgency !== 'FAILED').length;
        
        // Count urgent orders (OVERDUE, CRITICAL, WARNING urgency) - exclude FAILED
        const urgentCount = allShipments.filter(s => 
            ['PENDING', 'ASSIGNED', 'PICKED_UP'].includes(s.status) && 
        ...
        grid.innerHTML = 
            ...
            makeCard('In Transit', inTransitCount, 'purple', 'PICKED_UP') +
    ...
    function renderTable() {
        ...
        list.sort((a, b) => {
            ...
            if (!statusFilter) {
                const statusPriority = { 'PENDING': 0, 'ASSIGNED': 1, 'PICKED_UP': 2, 'DELIVERED': 4 };
        ...
        tbody.innerHTML = list.map(s => {
            ...
            const showUrgency = ['PENDING', 'ASSIGNED', 'PICKED_UP'].includes(s.status) && s.urgency && s.urgency !== 'NORMAL';
            ...
            const timeRemaining = (s.status === 'PICKED_UP' && s.dropoff_by)
            ...
            return '<tr class="hover:bg-slate-800 transition-colors border-b border-slate-700/50 ' + rowClass + ' group">' +
                     ...
                     (showUrgency ? '<span class="w-1.5 h-1.5 rounded-full ' + uConfig.bg + '"></span>' + uConfig.label : (s.status === 'PICKED_UP' ? 'In Transit' : s.status)) +
                     '</span>' +
                     '</td>' +
                     '<td class="px-6 py-4 whitespace-nowrap">' +
                     driverHtml +
