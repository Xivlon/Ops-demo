...
// Generated at: 2026-02-22T22:35:39.370Z
  ...
  <script>
    ...
    async function loadDrivers() {
        try {
            ...
            // Keep existing drivers list on error
        }
    }
        ...
        tbody.innerHTML = list.map(s => {
            ...
            if (s.status === 'PENDING') {
                const options = allDrivers
                    ...
                    .map(d => '<option value="' + d.id + '">' + (d.first_name + ' ' + d.last_name || d.email) + '</option>')
                    .join('');
            ...
            return '<tr class="hover:bg-slate-800 transition-colors border-b border-slate-700/50 ' + rowClass + ' group">' +
                     ...
                     fmtLoc(s, 'dropoff') +
                     '</div>' +
                     (s.pickup_at ? '<div class="text-[10px] text-slate-500 mt-1">Pickup: ' + new Date(s.pickup_at).toLocaleString([], {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'}) + '</div>' : '') +
                     '</td>' +
                     '<td class="px-6 py-4 whitespace-nowrap">' +
                     customerInfoHtml +
                     '</td>' +
