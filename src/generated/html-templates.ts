...
// Generated at: 2026-02-22T20:54:37.320Z
  ...
  <script>
    ...
    async function recalculateStats() {
      try {
        ...
        const res = await fetch('/api/drivers/stats/refresh', { method: 'POST' });
        
        if (!res.ok) throw new Error('HTTP error ' + res.status);
        
        const { data } = await res.json();
        
        if (data && data.driversUpdated !== undefined) {
    ...
    async function loadDriverStats() {
      try {
        const endpoint = currentDataSource === 'live' 
          ? '/api/drivers/stats/live' 
          : '/api/drivers/stats';
        ...
        if (freshnessUpdateInterval) clearInterval(freshnessUpdateInterval);
        freshnessUpdateInterval = setInterval(updateCacheFreshness, 10000);
