...
const DRIVER_STATS_HTML_PROMISE: Promise<string> = fetch(new URL('./driver-stats.html', import.meta.url)).then((r: Response) => r.text());
...
interface DriverStatsRequestBody {
  mode?: 'cached' | 'live';
  query?: string;
  params?: unknown[];
}
...
const worker: ExportedHandler<Env> = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const adminPin: string = env.ADMIN_PIN || "1234";
    const pinParam: string | null = url.searchParams.get("pin");
    ...
    if (path === "/api/driver-stats" && request.method === "POST") {
      ...
      let body: DriverStatsRequestBody;
      try {
    ...
    if (path.startsWith("/api/driver-timeline/") && request.method === "POST") {
      ...
      const driverIdFromPath: string | null = pathSegments[3] || null;
...
async function handleNeonQuery(request: Request, env: Env): Promise<Response> {
  try {
...
async function handleNeonQueryWithBody(env: Env, query: string, params: unknown[] = []): Promise<Response> {
  try {
    ...
    const result: QueryResult = await executeNeonQuery(connectionString, query, params);
...
async function handleLiveDriverStats(env: Env): Promise<Response> {
  try {
    const connectionString: string = env.DATABASE_URL;
    if (!connectionString) {
      return new Response(
...
async function handleRefreshDriverStats(env: Env): Promise<Response> {
  try {
    const connectionString: string = env.DATABASE_URL;
    if (!connectionString) {
      return new Response(
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ 
          error: `Failed to initialize driver stats table: ${err.message}`,
    ...
    try {
      ...
      const err = truncateError as Error;
      return new Response(
        JSON.stringify({ 
          error: `Failed to truncate driver stats table: ${err.message}`,
          code: "TRUNCATE_ERROR",
          details: err.message
        }),
        { 
    ...
    try {
      const insertResult = await sql`
        ...
        GROUP BY dp.id, dp.first_name, dp.last_name, dp.email, dp.is_online, dp.account_created_at
      `;
      console.log('✓ Driver stats data inserted, rows affected:', (insertResult as any).length || (insertResult as any).count || 0);
    } catch (insertError) {
      console.error('Error inserting into driver_stats table:', insertError);
      const err = insertError as Error;
      return new Response(
        JSON.stringify({ 
          error: `Failed to insert driver stats: ${err.message}`,
          code: "INSERT_ERROR",
          details: err.message
        }),
        { 
...
async function executeNeonQuery(connectionString: string, query: string, params: unknown[]): Promise<QueryResult> {
  try {
    const sql: NeonQueryFunction<false, false> = neon(connectionString);
    const rows: unknown[] = await sql(query, params);
...
async function testConnection(env: Env): Promise<Response> {
  try {
    ...
    const testQuery: string = "SELECT 1 as test_value, NOW() as current_time";
...
async function serveDashboard(pin: string | null): Promise<Response> {
...
async function serveDriverStats(pin: string | null): Promise<Response> {
  return new Response(await DRIVER_STATS_HTML_PROMISE, { 
    headers: { 
      "Content-Type": "text/html",
      "X-Content-Type-Options": "nosniff"
    } 
